...
{
    static chatGPT bot([]() {
        const char *envPath = getenv("LLM_MODEL_PATH");
        return envPath ? string(envPath) : string("../../models/qwen2.5-coder-7b-instruct-q5_k_m.gguf");
    }());

    return bot;
}

string build_user_id(const TcpConnectionPtr &con)
{
...
void CalculateTask::aiSearcher(const string &prompt)
{
    if (prompt.empty())
    {
        _con->sendInLoop("请输入 AI 问题，例如：\"7 介绍一下RAG\"\n");
        return;
    }

    auto &bot = get_chatbot();

    string answer = bot.chat(build_user_id(_con), prompt);
    _con->sendInLoop("[AI推理]\n" + answer + "\n");
    LogInfo("[LogInfo]ai inference");
}
...
void CalculateTask::process() 
{
	...
	else 
	{
        ...
        else if(7 == parser._id)
        {
            aiSearcher(parser._sought);
        }
		else 
		{  
